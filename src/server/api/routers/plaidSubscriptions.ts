...
export const plaidSubscriptionRouter = createTRPCRouter({
  create: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      return ctx.db.plaidSubscription.create({
        data: {
          name: input.name,
          cost: input.cost,
  ...
  getAll: protectedProcedure.query(async ({ ctx }) => {
    const subscriptions = ctx.db.plaidSubscription.findMany({
  ...
  count: protectedProcedure.query(async ({ ctx }) => {
    return ctx.db.plaidSubscription.count({
      where: { createdBy: { id: ctx.user?.id } },
    });
  }),
  ...
  update: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      return ctx.db.plaidSubscription.update({
        where: { id: input.id },
        data: {
          name: input.name,
          cost: input.cost,
  ...
  delete: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      return ctx.db.plaidSubscription.delete({
        where: { id: input.id },
      });
    }),

  getTotalMonthlyCost: protectedProcedure.query(async ({ ctx }) => {
    const subscriptions = await ctx.db.plaidSubscription.findMany({
      where: { createdBy: { id: ctx.user?.id } },
      select: { cost: true },
    });
  ...
  getUpcomingRenewals: protectedProcedure.query(async ({ ctx }) => {
    ...
    return await ctx.db.plaidSubscription.findMany({
      where: {
